...
frappe.ui.form.on("Hotel Check Out", {
  ...
  total: function(frm) {
    ...
    for (var i in frm.doc.room_bill) {
      temp_total_amount += frm.doc.room_bill[i].grand_total;
      if (frm.doc.room_bill[i].document_type == "Hotel Check In") {
        temp_stay_charges += frm.doc.items[i].grand_total;
      } 
    }
    for (var i in frm.doc.items) {
      temp_total_amount += frm.doc.items[i].amount;
      if (frm.doc.items[i].document_type == "Hotel Food Order") {
        ...
        .then(r => {
          ...
          frm.call("get_items").then(r => {
            if (r.message) {
              ...
              frm.doc.room_bill = undefined;
              frm.doc.items = undefined;
              frm.add_child("room_bill", {
                room_no: data[1].room,
                booking_days: data[0],
                price: data[1].price,
                grand_total: data[0] * data[1].price,
    ...
    frm.call("calculate_stay_days").then(r => {
      if (r.message) {
        if (r.message > -1) {
          ...
          for (var i in doc.room_bill) {
            if (doc.room_bill[i].room_no) {
              if (doc.items[i].document_type.includes("Hotel Check In")) {
                doc.items[i].booking_days = days;
                doc.items[i].grand_total = days * doc.items[i].price;
                frm.refresh_field("room_bill");
              }
            }
          }
